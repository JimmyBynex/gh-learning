...
- [ ] Documentation URL provided (DeepWiki, official docs site, readthedocs, etc.)
...
### Step 1 — Clone source and fetch documentation
...
**1b. Fetch documentation from user-provided URL**

Documentation sites cannot be cloned. Fetch systematically:

Step 1: Fetch the root URL the user provided. This is the index or overview page.

Step 2: Extract every internal link from that page that points to a sub-page within the same documentation site. Ignore external links, anchors, and asset URLs.

Step 3: Fetch each sub-page discovered in Step 2. If a sub-page contains further internal links not yet fetched, add them to the queue and fetch those too. Continue until no new pages remain.

Step 4: Save each fetched page as a `.md` file under `refs/wiki/`. Naming rules:
- Use the page's URL path as the filename, replacing `/` with `_` and stripping the domain
- Example: `https://deepwiki.com/obra/superpowers/hooks` → `refs/wiki/hooks.md`
- Add a header line at the top of each file: `<!-- source: <full-url> -->`

Step 5: After fetching, list all files saved under `refs/wiki/` and verify the count is reasonable (not just 1–2 pages for a large project). If fewer than 5 pages were fetched from a documentation site that visibly has more content, re-examine the link extraction and retry.

**1c. Optional: try GitHub wiki**

Some repos have a GitHub wiki. Attempt the clone — if it fails, skip silently:

```bash
git clone <github-url>.wiki.git <project-name>/refs/wiki-github 2>/dev/null || true
```

If it succeeds, merge content into `refs/wiki/` with filenames prefixed `github_`.

**Do not proceed to Step 2 until `refs/wiki/` contains the fetched documentation.**
