...
func viewRun(opts *ViewOptions) error {
	if opts.RepoArg == "" {
		return fmt.Errorf("repository argument is required(e.g. gh repo view owner/name")
	}
	...
	reposistory, err := api.GetRepository(client, repo)
	if err != nil {
		return err
	}
	printRepository(opts.IO, reposistory)
	return nil
}
